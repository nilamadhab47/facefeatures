...
def main():
    ...
    # カメラ準備 ###############################################################
    cap = cv.VideoCapture("/Users/nilamadhabsenapati/Documents/hand-gesture-recognition-using-onnx/hand_m.png")
    ...
    while True:
        ...
        if len(hands) > 0:
            ...
            for rect in rects:
                ...
                center, size, angle = rects_tuple
                angle_rad = np.radians(angle)
                box_half_size = 0.5 * np.array(size)


                box = np.array([
                    [center[0] - box_half_size[0] * np.cos(angle_rad) - box_half_size[1] * np.sin(angle_rad),
                    center[1] - box_half_size[0] * np.sin(angle_rad) + box_half_size[1] * np.cos(angle_rad)],
        ...
        # import pdb 
        # pdb.set_trace()
        cv.imwrite("madhab.png", debug_image)
